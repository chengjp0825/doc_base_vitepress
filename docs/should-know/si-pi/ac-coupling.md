...
在电子信号传输中，任何时域上的电压信号均可以被分解为直流分量（DC）和交流分量（AC）两部分。直流分量是信号的静态偏置电平，交流分量是其中随时间变化的部分，即瞬态变化的高频数据。

AC 耦合（交流耦合）是指在高速差分或单端传输链路中串联贴片电容器。利用电容器在频域上构成一阶高通滤波器的物理特性，它能够完全阻断频率 $f=0\text{ Hz}$ 的直流成分，仅允许高频的交流信号（即有效数据）通过并传输至下一级。


**那为何需要 AC 耦合呢？**

在跨板级（如 PCIe 接口）、跨系统或长距离线缆互连中，发射端（TX）与接收端（RX）物理隔离，客观上必然存在以下直流域的不匹配：

- 收发两端的原生工艺与共模电平不匹配：TX 端基于特定工艺输出的信号中心偏置为 1.2V（如 CML），而 RX 端内部判决器所需的最佳输入共模可能为 0.8V 甚至依靠内部自偏置。
- 地电位漂移 (Ground Shift)：互连系统间的地回流路径存在阻抗，导致两端参考地之间存在绝对电势差，直接影响信号的直流基准。

若采用直接连通的 DC 耦合，上述两端客观存在的直流压差将直接叠加在传输线上，强制拉偏 RX 端的实际输入电平。这会导致接收信号严重偏离判决器的线性区（偏离最佳眼图中心），极大压缩电压容限并导致误码率（BER）恶化，甚至可能击穿接收端芯片的内部晶体管。

引入 AC 耦合电容后，这种灾难性的直流压降被电容完全隔离。携带有效数据的交流信号在穿过电容到达 RX 端后，会受到 RX 内部端接网络（Termination Network，通常是由上拉/下拉电阻构成的戴维南等效结构）的作用，强制高频信号围绕 RX 本地所期望的、最干净的直流偏置点进行**电平重构**。

```mermaid
flowchart LR
    %% 样式定义
    classDef default fill:#1e293b,stroke:#475569,stroke-width:2px,color:#f8fafc
    classDef tx fill:#0f172a,stroke:#3b82f6,stroke-width:2px,color:#93c5fd
    classDef cap fill:#334155,stroke:#94a3b8,stroke-width:2px,color:#e2e8f0,shape:cylinder
    classDef signal fill:#1e293b,stroke:#10b981,stroke-width:2px,stroke-dasharray: 5 5,color:#34d399
    ...
    %% 连接与信号说明
    TX -- "复合信号 (交流数据 + 直流偏置)" --> C_AC
    C_AC -- "纯交流数据 (隔离直流)" ---> LINK
    
    %% 注释
    Note_TX["直流被隔离"] -.-> C_AC
    class Note_TX signal
```

上图所示，耦合电容位于发射端与传输线之间，承担了直流隔离的任务。
...
**总结**：AC 耦合通过电容隔离直流域，让发射端和接收端各自独立工作在最佳偏置条件下，是高速链路的"安全带"。
...
- [I2C 通信协议：开漏、上拉与总线时序](../../protocols/hardware/i2c.md)
